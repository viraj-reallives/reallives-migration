...
// export default function HomeschoolerAbout() {
...
//   return (
//     <section className={styles.about} aria-labelledby="homeschooler-about-heading">
//       <h1 id="homeschooler-about-heading" className={styles.heading}>
//         {about.heading}
//       </h1>
...
export default function HomeschoolerAbout() {
  ...
  return (
    <section className={styles.about} aria-labelledby="homeschooler-about-heading">
          ...
          <div className={Styles1.container_history_tab}>
            ...
            <div className={Styles1.history_second_container}>
              {[
                {
                  year: "2004",
                  desc: "The first widely adopted version of RealLives that introduced players to a simulation of life across the globe. With a basic UI and limited data, this version laid the foundation for what would become a powerful learning tool on global diversity.",
                },
                {
                  year: "2007",
                  desc: "An improved interface and broader country database made this edition more immersive. RealLives 2007 added more life events, cultural depth, and socio-economic details, bringing users closer to the everyday realities of people worldwide.",
                },
                {
                  year: "2014",
                  desc: "With enhanced graphics and better event logic, this version offered a more detailed life simulation experience. It allowed users to make impactful life decisions and see the consequences unfold over time. This version continued as a standalone desktop application.",
                },
                {
                  year: "2017",
                  desc: "This cloud-based version of RealLives, released in October 2017, brought the experience online. With a revamped interface and updated global data, it works seamlessly on all modern browsers and operating systems.",
                },
                {
                  year: "2025",
                  desc: "This cloud-based version of RealLives, released in October 2017, brought the experience online. With a revamped interface and updated global data, it works seamlessly on all modern browsers and operating systems.",
                ...
                <div key={idx} className={Styles1.card_realives_version}>
                  <p className={Styles1.reallives_text_version}>RealLives {item.year}</p>
                  <p className={Styles1.reallives_description_text}>{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        >
          <div className={styles.bioStack}>
            {team?.leader && (
              <div className={styles.bioRow}>
                <div>
                  {team.leader.role && <p className={styles.bioRole}>{team.leader.role}</p>}
                  {team.leader.name && <h2 className={styles.bioName}>{team.leader.name}</h2>}
                  {team.leader.bioParagraphs?.map((p, i) => (
                    <p key={`leader-bio-${i}`} className={styles.bioParagraph}>{p}</p>
                  ))}
                </div>
                {team.leader.imagePath && (
                  <div className={styles.bioImageWrap}>
            ...
            {team?.academicResearchCoordinator && (
              <div className={clsx(styles.bioRow, styles.bioRowReverse)}>
                {team.academicResearchCoordinator.imagePath && (
                  <div className={styles.bioImageWrap}>
                    <img src={team.academicResearchCoordinator.imagePath} alt="" className={styles.bioImage} />
                  </div>
                )}
                <div>
                  {team.academicResearchCoordinator.role && <p className={styles.bioRole}>{team.academicResearchCoordinator.role}</p>}
                  {team.academicResearchCoordinator.name && <h2 className={styles.bioName}>{team.academicResearchCoordinator.name}</h2>}
                  {team.academicResearchCoordinator.bioParagraphs?.map((p, i) => (
                    <p key={`arc-bio-${i}`} className={styles.bioParagraph}>{p}</p>
                  ))}
                </div>
              </div>
            )}
          ...
          >
            <h2 id="working-team-heading" className={Styles1.working_team_h1}>
          ...
          <div className={Styles1.interant_team_member_container}>
            <div className={Styles1.inerant_description_container}>
              <h1 className={Styles1.text_title_inerant}>Itinerant Team Members</h1>
              <div className={Styles1.descritpion_container_founder_2}>
                <p>Bob Runyan grew up in California's emerging Silicon Valley...</p>
                <p>After serving with the Peace Corps...</p>
                <p>The partnership that would transform RealLives...</p>
              </div>
            </div>
            ...
            <div className={Styles1.name_change_animation_box}>
              <div className={Styles1.name_box}>
                <div className={`${Styles1.slot} ${Styles1.slot_1}`}>
                  <p>Manisha Sathe</p><p>Vivek Rishi</p><p>Paresh Deshpande</p>
                  <p>Nikhil Jain</p><p>Purva Deshpande</p><p>Sateesh Khomne</p>
                  <p>Namita Pandya</p><p>Abhijit Kuljkarni</p><p>Aniruddha Bhide</p>
                  <p>Chaitanya Joshi</p><p>Tejas Chitale</p><p>Pinaki Dixit</p><p>Bob Runyan</p>
                </div>
                <div className={`${Styles1.slot} ${Styles1.slot_2}`}>
                  ...
                  <p>Aditi Thombre</p><p>Ramesh Garudkar</p><p>Sarita Karde</p>
                  <p>Anjani Pathak</p><p>Jaganntah Shelar</p><p>Sharvari Kulkarni</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {activeTab === 3 && (
        <div id="about-panel-3" role="tabpanel" className={styles.panel}>
          {globalSupport?.specialThanksHeading && (
            <div className={styles.supportSection}>
              <h2 className={styles.supportHeading}>{globalSupport.specialThanksHeading}</h2>
              {globalSupport.specialThanks?.map((item, idx) => (
                <div key={idx} className={styles.thanksRow}>
                  <div className={styles.thanksCard}>
          ...
          {globalSupport?.alwaysGratefulHeading && (
            <div className={styles.supportSection}>
              <h2 className={styles.supportHeading}>{globalSupport.alwaysGratefulHeading}</h2>
              <div className={styles.gratefulGrid}>
                {globalSupport.alwaysGratefulMembers?.map((member, idx) => (
                  <div key={idx} className={styles.gratefulCard}>
          ...
          {globalSupport?.thanksHeading && (
            <div className={styles.supportSection}>
              <h2 className={styles.supportHeading}>{globalSupport.thanksHeading}</h2>
              <div className={styles.thanksListGrid}>
                {globalSupport.thanksEntries?.map((entry, idx) => (
                  <div key={idx} className={styles.thanksEntry}>
