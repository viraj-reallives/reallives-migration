    ...
    <div className={styles.panel}>
      ...
      <div className={`${styles.gettingStarted} ${Style1.container_work_in_classroome}`}>
        <div>
          ...
          {supportingText.gettingStartedParagraphs?.map((text, i) => (
            <p key={i} className={styles.paragraph}>
              {text}
            </p>
          ))}
          ...
          <div className={styles.ctaRow}>
            <CtaLink
              to={supportingText?.gettingStartedCtaPath ?? "/contact"}
    ...
    <div className={styles.panel}>
      ...
      <div className={`${styles.splitSection} ${Style1.container_work_2}`}>
        <div>
          ...
          <ul className={styles.list}>
            {data.experienceBlock.listItems?.map((item, i) => (
              <li key={i}>{item}</li>
            ))}
          </ul>
        </div>
      ...
      <div className={`${styles.gettingStarted} ${styles.splitReverse}`}>
        ...
        <div>
          ...
          {data.empathyCanvas.steps?.map((step, i) => (
            <p key={i} className={styles.stepBlock}>
              <span className={styles.stepLabel}>{step.label}</span> {step.body}
            </p>
          ))}
    ...
    <div className={styles.panel}>
      ...
      <div className={styles.gridTwo}>
        <div className={styles.stack}>
          <section>
            <h2 className={styles.blockTitle}>{data.sdgMain.title}</h2>
            {data.sdgMain.intro && <p className={styles.paragraph}>{data.sdgMain.intro}</p>}
            {data.sdgMain.paragraphs?.map((p, i) => (
              <p key={i} className={styles.paragraph}>{p}</p>
            ))}
            {data.sdgMain.actionTagline && (
              <p className={styles.actionTagline}>{data.sdgMain.actionTagline}</p>
      ...
      <div className={`${styles.splitSection} ${Style1.container_work_2}`}>
        <div>
          ...
          <p className={styles.mutedLead}>{data.awarenessToAction.lead}</p>
          <ul className={styles.list}>
            {data.awarenessToAction.listItems?.map((item, i) => (
              <li key={i}>{item}</li>
            ))}
          </ul>
        </div>
      ...
      <section className={`${styles.benefitsSection} ${Style1.educational_benifit}`}>
        <div className={styles.benefitsInner}>
          <h2 className={styles.benefitsHeading}>{data.beyondClassroom.title}</h2>
          <ul className={styles.list}>
            {data.beyondClassroom.listItems?.map((item, i) => (
              <li key={i}>
                <p className={styles.paragraph} style={{ margin: 0 }}>{item}</p>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className={`${styles.gettingStarted} ${Style1.container_work_2}`}>
        <div>
          ...
          <div className={styles.ctaRow}>
            <CtaLink
              to={data?.closingCta?.ctaPath ?? "/contact"}
              className={`${styles.ctaButton} ${styles.ctaSdg} ${Style1.get_started_btn}`}
            >
              {data?.closingCta?.ctaText ?? "Get Started"}
            </CtaLink>
          </div>
        </div>
        {images.sdgGlobalAction && (
...
export default function HomeschoolerHome() {
  ...
  return (
    <div className={styles.page}>
      {labels.length > 0 && (
        <div className={`${styles.tabBar} ${Style1.tabBar_override_style}`}>
          {labels.map((label, index) => (
            <button
              key={label}
              className={`${styles.tab} ${activeTab === index ? styles.tabActive : ""}`}
      ...
      <div className={styles.homeTabPanels}>
        ...
        <div hidden={activeTab !== 1}>
          {extra[0] && <SkillsPanel data={extra[0]} images={images} />}
        </div>

        <div hidden={activeTab !== 2}>
          {extra[1] && <SdgPanel data={extra[1]} images={images} />}
        </div>
