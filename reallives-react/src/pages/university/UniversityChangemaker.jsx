...
/**
 * Helper component for handling internal/external links
 */
function CtaLink({ href, className, children }) {
  if (!href) return null;
  const isExternal =
...
export default function UniversityChangemaker() {
  ...
  // Split heading lines or provide a default
  const ctaHeadingLines = data.ctaSchoolHeading
    ? data.ctaSchoolHeading.split("\n").filter(Boolean)
  ...
  return (
    <div className={styles.page}>
      ...
      {/* JOURNEY SECTION */}
      {data.journeyHeading && (
        <h2 className={styles.journeyHeading}>{data.journeyHeading}</h2>
      )}
      ...
      {/* WHY SECTION */}
      {(data.whyJourneyMattersHeading || data.gaps?.length) && (
