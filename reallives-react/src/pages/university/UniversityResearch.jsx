  ...
  return (
    ...
    >
      ...
      <div className={styles.cardBody}>
        ...
        {/* ✅ WORKING BUTTON: Direct Navigate to route */}
        <button
          type="button"
          className={styles.exploreBtn}
          onClick={(e) => {
            e.stopPropagation(); // Card expansion prevent karne ke liye
            if (item.detail?.route) {
              navigate(item.detail.route);
            } else {
              onExploreClick(item.id); // Fallback: card expand karega agar route na ho
            }
          }}
        >
          {exploreText}
...
/**
 * Detailed View Component for a Selected Research Item
 */
function ResearchDetail({ research, item, onBack }) {
  const d = item.detail;
  const lb = research.detailPanelLabels;
...
/**
 * Main University Research Page Component
 */
export default function UniversityResearch() {
  const { research } = useSiteContent();
  const [expandedId, setExpandedId] = useState(null);
  ...
  return (
    <div className={styles.page} id="research">
      <header className={styles.hero}>
        ...
        {research.heroImagePath ? (
          <div className={styles.heroImage}>
            <img src={research.heroImagePath} alt="Research Hero" />
          </div>
        ) : null}
      </header>
